...
const Orderold = require('../models/orderModelold')

//@desc      Get all orders of customer
//@route     GET   api/v1/customer/allorders
//@access    Private
const getAllOrders = asyncHandler(async (req, res) => {
    const orders = await Orderold.find({ customer_id: req.user._id })
    if (!orders) {
        res.status(400)
...
const getOrder = asyncHandler(async (req, res) => {
    const order = await Orderold.findOne({ customer_id: req.user._id, _id: req.params.id })
    if (!order) {
        res.status(400)
...
const addOrder = asyncHandler(async (req, res) => {
    ...
    const order = await Orderold.create({
...
const updateOrderDetails = asyncHandler(async (req, res) => {
    ...
    const updateorder = await Orderold.findOneAndUpdate({ customer_id: req.user._id, _id: req.params.id }, {
...
const cancelOrder = asyncHandler(async (req, res) => {
    ...
    const cancelorder = await Orderold.findOneAndUpdate({ customer_id: req.user._id, _id: req.params.id, isCancled: false }, {
        isCancled: true
    }, {
        new: true
    })
